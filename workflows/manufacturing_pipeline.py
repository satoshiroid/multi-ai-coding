...
APP_PIPELINE: tuple[Stage, ...] = (
    ...
    Stage(
        ...
        instruction_hints={
            Domain.SOFTWARE: (
                ...
                "【最重要】ブラウザだけで完結できる機能なら、バックエンド無しの"
                "**自己完結クライアントサイド静的Webアプリ**（HTML/CSS/JSのみ、"
                "エントリは index.html）を選ぶこと。Docker/サーバ/フレームワークは"
                "本当に必要な場合のみ。静的サイトとしてそのまま公開・実行できる構成にする。\n"
    ...
    Stage(
        ...
        instruction_hints={
            Domain.SOFTWARE: (
                "【最重要】要件で指示された“その機能”を忠実に・実際に動く形で実装してください。"
                "汎用サンプル（ToDo/カウンター等）やプレースホルダで代替してはいけません。"
                "要件の中核機能（例: 入力された2つの対象を横並び比較する等）を必ず実装すること。\n"
                "ブラウザだけで動く**自己完結の静的Webアプリ**にし、エントリは `index.html`"
                "（CSS/JSはインラインまたは最小ファイル）。サーバ不要で、開く/静的配信で"
                "そのまま動作すること。\n"
                "artifacts.files: {\"ファイルパス\": \"コード\"} 形式。index.html を中心に、"
                "機能が成立する範囲で完結させる（プレースホルダ禁止。冗長な骨組みより"
                "“動く中核機能”を優先）。\n"
                "artifacts.setup: 実行手順（簡潔に）。"
            ),
        },
    ),
    Stage(
